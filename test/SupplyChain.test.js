...
describe("SupplyChain", function () {
  async function deploySupplyChainContract() {
    ...
    const supplyChain = await SupplyChain.deploy(
      usersContract.getAddress(),
      products.getAddress()
    );
    return { supplyChain, usersContract, products, owner, name, email };
  }
  ...
  describe("Products", function () {
    it("Manufacturer adds a product", async function () {
      const { supplyChain, _, products } = await loadFixture(
        deploySupplyChainContract
      );
      ...
      // await supplyChain.connect(admin).addProductType(newProductType);
      ...
      let tx0;
      for (let i = 0; i < 300; ++i) {
        tx = await supplyChain.connect(manufacturer).addProduct(newProduct);
        if (i == 0) {
          tx0 = tx;
        }
      ...
      console.log("gasLimit", tx0.gasLimit);
      console.log("tx0.gasPrice", tx0.gasPrice);
      console.log("tx300.gasPrice", tx.gasPrice);
      console.log("avg.gasPrice", sum / BigInt(1000));
